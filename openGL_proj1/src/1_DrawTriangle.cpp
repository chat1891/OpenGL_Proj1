...
static unsigned int CreateShader(const std::string& vertexShader, const std::string& fragmentShader)
{
    ...
    if (!success)
    {
        char* infoLog = (char*)_malloca(success * sizeof(char));
...
int DrawTriangle_main(void)
{
    ...
    //-the vertex value will be normalized <1 to fit in screen
    float positions[6] = {
        -0.5f, -0.5f,
         0.0f,  0.5f,
         0.5f, -0.5f
    ...
    //-first parameter: layout (location = 0). This sets the location of the vertex attribute to 0 and since we want to pass data to this vertex attribute, we pass in 0.
    ...
    //-for better practice, prob have a strut set up, and use offset micro to figure it out, instead just write (const void*)8
