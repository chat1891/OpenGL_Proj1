...
int main(void)
{
    ...
    //use Core profile, has to create vertex array object. 
    ...
    //So if VAO 0 is bound in the core profile, you should not call any function that modifies VAO state.
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        /*
        * change from
        bind shader => bind vertex buffer => set up vertex layout => bind index buffer => issue draw call
        to 
        bind shader => bind vertex array => bind index buffer => issue draw call
         bind vertex buffer + set up vertex layout becomes bind vertex array, because it contains all the state we need
        ...
        */
		...
		/*
        when bind vertex array, and bind the buffer, nothing links the two
        glVertexAttribPointer specifiy index 0/1/2 of this vertex array is going to be bound to currently bound GL array buffer
        which means it links VBO TO VAO
        That's why only need to bind vertex array here
        */
        GLCall(glBindVertexArray(VAO));
        GLCall(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO));
